...
func Seven5Content(h Handler, urlPath string, projectName string) {
	...
	if projectName!="" {
		h.ServeMux().HandleFunc(fmt.Sprintf("%ssecret/%s/", urlPath, projectName), secretHandler)
	}
}
...
//code.  If the project name is "" then the secrets.json file will not be read and
//no secrets will be visible from the client side (nothing at /seven5/secret)
...
//secret is used to hold the mapping from keys to values used for secrets to be exposed by the server
//if requested by the client.
var secret map[string]string

//secretReader reads the filename provided in the project go directory to get the list of secrets.
//The file's content should be a json map from strings to strings.  This is usually only one run
//time.
func secretReader(projectName string, filename string) error {
...
//secret handler responds to requests for a secret or gives 404. secrets are placed in secret.json
func secretHandler(response http.ResponseWriter, request *http.Request) {
	...
	if parts[targ] == "" {
		http.Error(response, "no secret requested", http.StatusBadRequest)
		return
	}
	...
	if secret==nil {
		...
		if err:=secretReader(parts[projectName], "secret.json"); err!=nil {
			secret=nil //try again if they fix the formatting
			http.Error(response, fmt.Sprintf("failed reading secrets file: %s", err), 
				http.StatusInternalServerError)
			return
		}
	}	
	
	s, ok:= secret[parts[targ]]
	if !ok {
		http.Error(response, "secret not found", http.StatusNotFound)
	}
